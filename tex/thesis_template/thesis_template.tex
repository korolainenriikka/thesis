...
Uniformizing the structure of handwritten fossil catalogues maintained at natural history museums worldwide exhibits a great 
...
and a pipeline for uniformizing the dental notation present in 
...
continents, speciments residing in archives of institutions have been recorded to digital, public databases~\cite{uhenCardCatalogsComputers2013}.
...
Specifically, this work uses as input data both scan images of tablular fossil catalogues
...
uniformizing dental notation. Each element description is processed 
word-by-word, and for each word, only those recognized as tooth markings
by using a regular expression on the Azure reading are given to tooth marking
...
In this chapter, background on paleoenvironmental reconstruction is overviewed, focusing on methods for dental fossil data.
...
can contain subjective interpretations, and transfer functions allow analyzing larger sets of species occurences than would 
...
and temperature~\cite{oksanenHumboldtianApproachLife2019} can be learned based on dental lophs, number of cutting blade shapes, and hypsodontry, the height of the
...
terms are 'mand' for mandibles, and letter 'D' for decidous teeth.
...
animal, not the observer. Mammal teeth are always symmetrical from one side to the other, thus every tooth always has the equivalent other-jaw counterpart. Conversely, upper and lower jaw teeth are not always symmetric. The distance of a tooth from the throat 
...
In each of these systems, a superscipt, underline, or capital letter denotes upper jaw, and conversely subscript, overline or lowercase letter denotes the lower jaw. The line can be thus interpreted to picture the other jaw bone and the number the bone the tooth is from.
...
also known as generalization performance.
...
Neural network architectures are alternative ways of constructing networks of layers with different computations for cases where the standard variant presented in Equation~\ref{eq:fc_layer} is not ideal. Alternative layer types are used to make the model pay more attention to desired 
...
Transformation invariance states that morphing an image keeps the semantic meaning: for instance,
...
hyperbolic tangent activation popular at the time with the ReLU function (Equation~\ref{eq:relu})~\cite{alexnet}.
...
Following the success of increasingly deep networks~\cite{vgg,googlelenet}, the question arised of whether 
...
the deeper network contains in itself all the shallower variants, and thus the resulting accuracy can never 
...
connection makes approximating the identity mapping very easy by simply setting all parameters but the skip connection to zero, the problem is completely eliminated. The end results were impressive: 
...
variable is the input~\cite{goodfellow}. To avoid situations where the network could simply copy the values from 
...
another computation from $h$ to the input, thus an autoencoder is a function $x\to h\to x$~\cite{goodfellow}.
...
Likely due to the fact that the problem has attracted little attention, 
...
Museum of Kenya, for which this work is a continuation. Therefore, the experiments in this thesis will 
...
recognizing which can be seen as analogous to under- or overlinings in the dental markings.
...
with a deep convolutional network. This proves that while less popular, approaches based on unsupervised pretraining  are worth experimenting with.
...
Lastly, a very interesting application domain of reading characters used in ancient historical scripts written 
...
the fact that object detection requires better image resolution to work well, therefore requiring more computational resources.
