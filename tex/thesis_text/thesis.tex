...
Given the chosen system and a regular expression classifier for words to tooth markings 
and other words, five classifiers are needed: upper or lower jaw classifier, 
tooth type classifier, and two downstream index number classifiers for numbers one to three and one to four. The experimental 
...
Train, validation and test split was selected by creating a validation dataset of roughly 200 samples to one sample classification
error not change test error by more than 0.5\%. For the test set, 100 samples were kept. This resulted in 
train set sizes, including the images generated to balance classes, of 905 samples for upper or lower, 
2071 for M, P or I, 1521 for index one to three, and 2026 for index number one to four.
...
For data labeling, the outputs from Azure Vision were heavily utilized. For 
