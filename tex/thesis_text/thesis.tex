...
% inspired by tibetan but no direct previous work exists so a as simple as possible solution was chosen
...
% 2. building models for the pipeline
% (how the solution was evaluated, what were the results of this evaluations. scope and limitations)
% three tasks: tooth type up low index number. assume train from scratch makes no sense~\cite{cs231n_transfer_learning},
% find best base model and layer freezing setup by running multiple model + freezing combinations.
% experiments can be found in full in [link]. found: best is alexnet without freezing w. accuracy xxx.
% other models mpi 1234 123 best accuracies are yyy zzz ccc respectively, x y z percent have confidence > 99.7, human level so need no manual verifying
% limitations: lost context, errors in bounding box cause that i cannot classify everything
...
Section~\cite{sect:ecometrics} highlights dental ecometrics, a method for constructing mappings from dental elements to environmental traits to motivate the 
need for representative and accurate tooth fossil datasets. As further background on the data, Section~\cite{sect:mammal_teeth} presents the mammal teeth 
...
of such case is found in Figure~\cite{image:3dkernel}.
...
The architectures of the AlexNet, GoogLeNet and VGG models are summarized in Figure~\cite{image:famouscnns}.
...
task in transfer learning, as discussed in Section~\cite{sect:transfer_learning}.
...
equal dimension as the one-head version. The self-attention operation with multiple heads is summarized in Figure~\cite{fig:multi-head-self-attention}.
...
presented in Section~\cite{sect:training}: starting training with a neural network setup, 
...
This chapter is organized as follows. In Section~\cite{sect:problem-formulation}, the main problem is formulated as a set of $X\to y$ 
...
studied literature, in Section~\cite{sect:building-models}.
...
A sample of the catalogues is presented in Figure~\cite{image:cataloguesample}.
...
marking. The notation used in the end result will be the digital notation presented in Table~\cite{table:jaw_notation}.
...
obtained by hand-annotating as no perfect solution is found in this work, is presented in Figure~\cite{image:goal}. 
...
illustrated as a flowchart in Figure~\cite{fig:pipeline}, has 
...
catalogues using the first two steps on the processing pipeline in Figure~\cite{fig:pipeline}:
...
method used will be presented in Section~\cite{sect:aug}. The dataset sizes and samples counts per class 
...
A small sample of the training dataset of premolars is presented in Figure~\cite{image:samples}.
...
be found in Figure~\cite{image:noise}.
...
is presented in Figure~\cite{image:augmented}.
...
according to results from related work, reviewed in Section~\cite{sect:same_solution}.
...
Analyzing Table~\cite{tab:model-info}, EfficientNet models consistently outperform 
...
For the transfer method, the reviewed work in Table~\cite{tab:model-info} has a big variety of approaches, 
...
number of epochs chosen as an average value among the reviewed works, are summarized in Table~\cite{table:hyperparameters}.
...
The experiment outcomes are collected to Table~\cite{table:results}, and 
...
which strongly contradicts results of many reviewed works in Section~\cite{sect:same_solution},
suggesting that a model found to be better in one study can inform little on its performance on another target task.
The generalization capability of all models seems to be good, as seen in Figure~\cite{image:diff_hist}:
...
changing the initial steps of the learning trajectory, which is important in neural network training~\cite{transferlearning_survey}.
 As seen in Table~\cite{table:random_seed_sensitivity}, the largest difference occurred with one layer 
...
changed by 6,49\%, as seen in Table~\cite{table:hyperparameter_sensitivity}. Notably, the best accuracy was outperformed by as much as 4,54 percentage points 
...
also the only larger consensus among studies reviewed in Section~\cite{sect:same_solution}.
As is highlighted in the plot of layers trained against resulting accuracy in Figure~\cite{image:accuracy_vs_freezing},
...
Figure~\cite{image:meme}, one can simply say "we need to go further"; the way forward is simply exposing the model to more examples, 
