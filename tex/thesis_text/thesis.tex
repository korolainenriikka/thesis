...
This chapter presents the results of the experiments. Most of the experiments were completed using the 
upper and lower jaw classification problem, since initial experiments suggested it to be the most challenging 
classification tasks. The experimentation was limited to testing out different base models, AlexNet, EfficientNet, 
DenseNet, Vision Transformer and Resnet, and freezing setups: training 1, 5, 10 or all layers.
The results of these experiments hightlighted three major results. First, the best architectures were AlexNet and the Vision 
Transformer with best validation accuracies of xx and xx, respectively, while EfficientNet performed poorly. Second, tests on changes in results 
...
To estimate how much the results depended on the optimum found in training and how much on architecture, 
the best architecture AlexNet was retrained with one hyperparameter changed, and the resulting 
change in accuracy suggests that comparing accuracies when target data is small is highly inaccurate. 
...
changing the initial steps of the learning trajectory, which is important in neural network training \cite{transferlearning_survey}.
 The largest difference occurred with one layer 
trained, and the second seed had the validation accuracy increased by 9,09 percentage points. Then, hyperparameter change 
by changing batch size from 64 to 32, also a random choice, was tested with the AlexNet architecture, and the results 
changed by 6,59\% (TODO difference col, check this). Notably, the best accuracy was outperformed by as much as 4,5 percentage points 
...
resources to repeat the experiments with different hyperparameter setting would be required to draw more fine-grained conclusions on the 
...
% so this could show that it might be that initial examples are important, 
% so generalist images to start is good, that is where transfer power comes from
...
% a strong result, happened with every model
% results align with other ocr studies.
The most clear result from the experiments is that freezing in every case deteriorated the resulting accuracy.
...
more trainable parameters did not increase accuracy. This result of freezing deteriorating performance aligns with the 
other studies reviewed in Section \ref{sect:same_solution}, where the detrimental effect of freezing was one of the only 
results with a larger consensus over many studies. 
...
% 	-> seems false, training the whole network works better.
...
examples is re-used from another training run, allowing for training longer and therefore seeing more examples. Whether this is 
the only reason to why transfer works, or if part of it is due to the hypothesis that general-style initial examples 
set the learning trajectory on a better path \cite{transferlearning_survey} cannot really be verified since one can never obtain 
a training set as large as ImageNet on any specialist domain. Taking the idea that transfer utility is only about 
...
due to the transfer distance being shorter or if the reason is the sheer size of the dataset. Again, this idea is hard 
to test for the fact that finding other datasets as large as ImageNet is challenging. Negative transfer presented in the transfer learning 
survey \cite{transferlearning_survey} also fits in this scheme: to see more examples, it is intuitive that all examples need to 
be informative about the problem at hand. Summarizing all this on finding better models for a variety of tasks 
can be boiled down to one phrase, and returing to the GoogLeNet inspiring meme presented in 
...
and re-using the previously learned is a way to achieve this with limited computational resources.
...
\subsection{Future work on digitizing the fossil catalogues}
