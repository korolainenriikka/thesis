...
The primary motivation for the introduction of convolutional layers was to find a way to encode a prior 
on image processing in general to the network architecture: force the network, by adjusting its computational 
algorithm, to pay attention to particular aspects while ignoring others. By constraining the problem with 
this prior assumption allows reducing the network parameter count per layer, which frees up 
...
% \cite{princebook}
The two main pieces of prior knowledge are invariance to transformations and local relatedness of pixels.
