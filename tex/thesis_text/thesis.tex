...
base model variants are listed starting from the best for brevity. The Tibetan detection article \cite{tibetan_ocr}
...
\cite{10goelGujarati}               & 96.5\%                 & EfficientNet, InceptionV3, ResNet101 & Add 2 FC layers + output layer, no freezing \\ \hline
...
from the project output and characteristics of the scan images. After that,
the input type, image of sentence or word is chosen, followed by a formalization
of the model output probability vector.
...
The fossil catalogue images are hand-drawn tables with mostly uniform columns, such 
...
A sample of the catalogues is presented in Figure TODO.
...
to be incorrect with prior knowledge on characters present in tooth notation (M, P, I, C, 1, 2, 3 and 4).
...
The aim of the experiments in this work is to find an approach for fixing thse errors.
